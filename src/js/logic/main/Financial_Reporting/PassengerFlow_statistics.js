var Financial_Reporting_PassengerFlow_statistics = (function () {
  var table = {
    // 数据集
    ...
    ajax: function (timer) {
      /* 添加加载状态 */
      $('.content-table-det').html(template('content/Financial_Reporting/PassengerFlow_statistics_table'))
      var _this = this;
      var parms = {
        "start_date": timer.s,
        "end_date": timer.e
      ...
      $.ajax({
        type: 'POST',
        url: BaseSet.url + '/financeTouristCentreReport/listFinancePassengerFlow',
...
//    Content(this.data, this)
      $('.content-table-det').html(template('content/Financial_Reporting/PassengerFlow_statistics_table', this.data))
      this.bind()
    },
    // 数据绑定
    bind: function () {
      ...
      $('#save').on('click', function() {
        ...
        $.ajax({
          type: 'POST',
          url: BaseSet.url + '/financeTouristCentreReport/saveOrUpdateFinancePassengerFlowBatch',
          contentType:"application/json;charset=utf-8",
  ...
  var one = {
    ...
    handle: function () {
      this.data.build = {
        ...
        line: 1, // 第几行
        height: '100%', // 占全屏幕的多高
        template: 'Financial_Reporting/PassengerFlow_statistics' // 调用模板名称
      }
