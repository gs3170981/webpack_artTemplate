var Sales_Report_TourOperatorSale_Total = (function () {
  var table = {
    // 数据集
    ...
    ajax: function () {
      /* 添加加载状态 */
      $('.content-table-det').html(template('content/Sales_Report/TourOperatorSale_Total_table'))
      var _this = this;
      $.ajax({
        type: 'GET',
        url: BaseSet.url + '/marketingDepartmentReport/travelAgencteamTotal',
...
//    Content(this.data, this)
      $('.content-table-det').html(template('content/Sales_Report/TourOperatorSale_Total_table', this.data))
      this.bind()
    },
    // 数据绑定
    bind: function () {
  ...
  var one = {
    ...
    handle: function () {
      this.data.build = {
        ...
        line: 1, // 第几行
        height: '100%', // 占全屏幕的多高
        template: 'Sales_Report/TourOperatorSale_Total' // 调用模板名称
      }
