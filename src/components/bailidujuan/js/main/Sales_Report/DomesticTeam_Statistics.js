var Sales_Report_DomesticTeam_Statistics = (function () {
  var table = {
    // 数据集
    ...
    ajax: function (timer) {
      /* 添加加载状态 */
      $('.content-table-det').html(template('content/Sales_Report/DomesticTeam_Statistics_Table'))
      var _this = this
      var parms = {
      ...
      $.ajax({
        type: 'POST',
        url: BaseSet.url + '/marketingDepartmentReport/domesticTeamStatistics',
...
//    Content(this.data, this)
      $('.content-table-det').html(template('content/Sales_Report/DomesticTeam_Statistics_Table', this.data))
      this.bind()
    },
    // 数据绑定
    bind: function () {
  ...
  var one = {
    ...
    data: {
      ...
      province:[
        ...
        '上海','江苏','安徽','福建','江西','山东','河南','湖北','湖南',
        '广西','海南','重庆','四川','贵州','云南','西藏','陕西','甘肃',
        '青海','宁夏','新疆','台湾','香港','澳门','浙江','广东'
      ],
      timer: {
        s: '1',
        e: '1'
    ...
    handle: function () {
      this.data.build = {
        ...
        line: 1, // 第几行
        height: '100%', // 占全屏幕的多高
        template: 'Sales_Report/DomesticTeam_Statistics' // 调用模板名称
      }
